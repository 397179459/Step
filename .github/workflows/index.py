...
def main(user, passwd, step):
    ...
    #     print("用户名或密码填写有误！")
    #     return
    # print(user, passwd, step)
    # if 1 == 1:
    #     print("已设置为随机步数")
    #     step = str(random.randint(98698, 98999))
    login_token = 0
    login_token, userid = login(user, password)
    if login_token == 0:
        # print("{0}登陆失败！！！".format(user))
        return "[{0}] 登陆失败！\n".format(user)
    # else:
        # print("{0}登陆成功。。。".format(user))
    t = get_time()
    ...
    # print(response)
    result = f"@@@ {user}:update( {step} )" + response['message']
    # print(result)
    # push_wx(result)
    return result
...
# 推送消息到微信
def push_wx(msg):
    token = '8312fe75e0534125aab76b7e867cd8c7' #在pushplus网站中可以找到
...
def push_wx_Server(msg):
    ...
    requests.post(url, data={'text': msg, 'desp': msg})
...
# def main_handler(event, context):
    ...
    # user1 = '13313365975'
    # passwd1 = 'wangLEI1997'
    # step1  = str(random.randint(98698, 98999))
    dic1 = {'username0': '13313365975', 'pwd0': 'wangLEI1997', 'step0': str(random.randint(99200, 99600))
            ,'username1': '18905692497', 'pwd1': 'Patricia27', 'step1': str(random.randint(30000, 40000))
            # 10.22~11.22[13028085217]
        #    ,'username2': '13028085217', 'pwd2': '35234192q', 'step2': str(random.randint(30000, 40000))
           
            }
    # pushMsg = 'test push'
    pushMsg = ''
    for i in range(int(len(dic1) / 3)):
        # print(i)
        uid = 'username{0}'.format(i)
        upwd = 'pwd{0}'.format(i)
        ustep = 'step{0}'.format(i)
